...
import { LoanServiceProxy, CreateOrEditLoanDto, GetLoanForEditOutput, MiscFunctionServiceProxy, StatusDto } from '@shared/service-proxies/service-proxies';
...
export class EditLoanDialogComponent extends AppComponentBase implements OnInit {
    ...
    ngOnInit(): void {
        ...
        if (this.id) {
            this._loanService.getViaIdForEdit(this.id).subscribe((result: GetLoanForEditOutput) => {
                this.loan = result.loan;
                this.interestStartDate = this.loan.interestStartDate ? (this.loan.interestStartDate as any).toDate() : null;
                this.maturityDate = this.loan.maturityDate ? (this.loan.maturityDate as any).toDate() : null;
                this.appRef.tick();
                this.refreshSelect(this.pawnTicketSelect);
                this.refreshSelect(this.statusSelect);
            });
